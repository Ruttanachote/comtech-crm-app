  ...
  const cleanupKeycloakHash = () => {
    ...
    if (hash && hash.length > 1) {
      try {
        ...
        if (hasKeycloakParams) {
          keycloakParams.forEach((param) => hashParams.delete(param));
          const newHash = hashParams.toString();
          const urlWithoutHash = window.location.href.split('#')[0];
          const newUrl = newHash ? `${urlWithoutHash}#${newHash}` : urlWithoutHash;
          window.history.replaceState(window.history.state, '', newUrl);
        }
      } catch (error) {
        console.error('Error cleaning hash params', error);
      }
    }
    ...
    if (search && search.length > 1) {
      try {
        ...
        const hasKeycloakParams = keycloakParams.some((param) => searchParams.has(param));

        if (hasKeycloakParams) {
          keycloakParams.forEach((param) => searchParams.delete(param));
          const newSearch = searchParams.toString();
          const urlWithoutSearch = window.location.href.split('?')[0];
          const newUrl = newSearch ? `${urlWithoutSearch}?${newSearch}` : urlWithoutSearch;
          // Preserve hash if exists
          const currentHash = window.location.hash;
          window.history.replaceState(window.history.state, '', newUrl + currentHash);
        }
      } catch (error) {
        console.error('Error cleaning query params', error);
      }
    }
  };
